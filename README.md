...
- **[attacker](skills/attacker/)** —— 攻击产品的*真实可观测行为*，或红队一个想法/论点/方案：一个全新、与 TDD 独立的 subagent 在声明范围内攻击，只把可复现、已证实的破坏记成 attack record，由确定性校验器把关；与 loop-constructor 配对（攻击→修复→再攻击）。
...
- **[humanizer-academic](skills/humanizer-academic/)** —— 重写 AI 生成的严肃文本（中 / 英 / 混合），**两种模式**：学术论文 与 严肃科普。**好在哪：** **先判定、能不动就不动**（读着像人就原样返回 —— 治好「过度修改」的误伤）；模式决定保留什么腔调、什么才算 AI 痕迹（设问/类比在科普是手艺、在论文是失格；数据三连/「significant」/编号小节在论文是常态、不是 AI 痕迹）；检测器重做成**低误报的「水货」探测器 + 诊断盘**（不是 AI 分类器 —— 真实数据校准证明现代严肃 AI 与人类文本在正则/统计特征上重叠），真正的判官是独立盲审。真实语料 eval（27 篇真人 + 20 篇 AI）：**人类文本 0/27 被过度修改或杜撰，0 杜撰**。**v3.1 完成度升级**（盲审标尺重做为 per-mode：学术轨 + 科普轨，各加「完成度」维度 + 整篇长文用例 + reserve-5 严判）：**整篇完成度（盲审 5 分制）学术 4.00→4.83、科普 4.17→4.83**，FP=0 / 杜撰=0 不变，并经**独立留出集攻击两轮全 clean 验证（泛化到 中/英/混合，样本外不误伤、不杜撰、保留科普手艺）**。
...
- **[attacker](skills/attacker/)** —— 攻击一个产品的*真实可观测行为*，或红队一个想法/论点/方案（辩论反方）：一个**全新、与 TDD 独立**的 subagent 在声明的攻击范围（`--scope`/`--out-of-scope`）内攻击，只把可复现、已证实的破坏记成 attack record。**好在哪：** 产品模式只看需求 + 可观测行为，**绝不看实现 / TDD 套件 / 作者框架**（否则会重新继承构建者的盲点 → 假绿），想法模式接过论点但独立批判；PROVE-OR-FLAG —— 只记已证实可复现的破坏，绝不杜撰；每条记录带可审计的 `independence_attestation`，由确定性校验器（`validate_attack_records.mjs` + 发布门）把关；与 [loop-constructor](skills/loop-constructor/) 配对成 攻击→修复→再攻击 的循环。这条独立新鲜测试组正是「闭环会骗人」原则的执行臂 —— 连跑 5 轮，每轮都揪出构建者 20/20 自测漏掉的「绿但错」。
...
工程化的诚实要求把没堵死的也写出来 —— 这正是「闭环会骗人」的延伸（见上）。流水线最近一轮分段 debug（guidance 触发式提问、conductor 接入 attacker）后的当前残留项：

- **guidance 的上下文充分性探测器是「种子」，不是「裁判」。** 触发提问的关键词探测器两个方向都可能被骗（堆砌规格词汇会被读成「充分」，同义改写会被读成「不充分」）。已按设计缓解 —— 规则把它当种子，由 agent 对「实质内容」的判断作最终裁判 —— 但并非确定性地堵死。
- **conductor 的「attacker 须在复审通过后才触发」是约定 + 不变量校验，尚未运行时硬联锁。** 该次序由规则文本 + conductor-log 的 `min(re-audit, battery)` 不变量保证；尚未在 conductor 自己的 `round6` 门里做成硬性机器门（后续项）。
- **验证是渐近的，不是证明。** 独立 maker/checker 对抗组每一轮仍可能再揪出一个「绿但错」；我们在堵死所有「已证实」的漏洞后收手，而非宣称完美。
- **loop-constructor 的 D6「完成度优先 / 迭代优先」节奏是「指南」，非 linter 强制（按需求刻意选择）。** 设计可声称 `completeness_first` 却配高 caps + retry + 烟雾检查 —— linter 抓不到这个错标；由 fresh-reader 的 cadence 框（含量化护栏：completeness-first ≈ caps ≤4）+ maker/checker 把关，残留如实记录而非假装堵死。
