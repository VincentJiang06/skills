...
- **[attacker](skills/attacker/)** —— 攻击一个产品的*真实可观测行为*，或红队一个想法/论点/方案（辩论反方）：一个**全新、与 TDD 独立**的 subagent 在声明的攻击范围（`--scope`/`--out-of-scope`）内攻击，只把可复现、已证实的破坏记成 attack record。**好在哪：** 产品模式只看需求 + 可观测行为，**绝不看实现 / TDD 套件 / 作者框架**（否则会重新继承构建者的盲点 → 假绿），idea 模式接过论点但独立批判；PROVE-OR-FLAG —— 只记已证实可复现的破坏，绝不杜撰；每条记录带可审计的 `independence_attestation`，由确定性校验器（`validate_attack_records.mjs` + 发布门）把关；与 [loop-constructor](skills/loop-constructor/) 配对成 攻击→修复→再攻击 的循环。这条独立新鲜测试组正是「闭环会骗人」原则的执行臂 —— 5 轮电池每轮都揪出构建者 20/20 自测漏掉的「绿但错」。
