...
|---------|----------------|--------|
| Exit 0, 0 LOST | Every original line survived | Pass |
| Exit 1, N LOST | N lines disappeared without a near-match | Investigate each LOST line |

For each LOST line, classify:
...
Do NOT mark a LOST line as "accepted" without naming which category it
falls into. The lossless guarantee is the skill's contract with the user;
silently waving losses is the failure mode.
...
Done.
  ...
  ✓ Lossless check: 0 LOST lines (or: N accepted rewrites — list category for each)
