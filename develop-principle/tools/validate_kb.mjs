...
const TEST_STRATEGY_FIELDS = ["schema_version", "matrix_id", "title_zh", "title_en", "levels", "reference_ids"];
const TEST_STRATEGY_LEVEL_FIELDS = ["level", "name", "goal_zh", "goal_en", "typical_checks"];
const TDD_WORKFLOW_FIELDS = ["schema_version", "workflow_id", "title_zh", "title_en", "steps"];
const TDD_WORKFLOW_STEP_FIELDS = ["id", "phase", "action_zh", "action_en", "outputs"];
const TDD_PLANNING_FIELDS = ["schema_version", "matrix_id", "title_zh", "title_en", "phases", "reference_ids"];
const TDD_PLANNING_PHASE_FIELDS = ["phase", "required_outputs", "stop_condition"];
...
const EDGE_TYPES = new Set(["contains", "contradicts", "decomposes_to", "defines", "depends_on", "measures", "supports", "uses"]);
const TDD_PHASES = new Set(["red", "green", "refactor", "harden"]);
...
const testStrategyMatrix = readJson("testing/test_strategy_matrix.json");
const tddWorkflow = readJson("testing/tdd_workflow.json");
const tddPlanningMatrix = readJson("testing/tdd_planning_matrix.json");
...
if (qualityGateSet?.gate_set_id !== "quality.develop_principle") errors.push("kb_quality_gates gate_set_id mismatch");
...
if (querySuite?.suite_id !== "query_effectiveness.skill_design_requirements") errors.push("query_effectiveness_cases suite_id mismatch");
...
assertExactFields("test strategy matrix", testStrategyMatrix?.matrix_id ?? "unknown", testStrategyMatrix ?? {}, TEST_STRATEGY_FIELDS);
if (testStrategyMatrix?.schema_version !== "1.0.0") errors.push("test_strategy_matrix schema_version must be 1.0.0");
if (testStrategyMatrix?.matrix_id !== "testing.skill_strategy_matrix") errors.push("test_strategy_matrix matrix_id mismatch");
assertUniqueArray("test strategy matrix", testStrategyMatrix?.matrix_id ?? "unknown", "reference_ids", testStrategyMatrix?.reference_ids ?? []);
for (const refId of testStrategyMatrix?.reference_ids ?? []) {
  if (!knownRefIds.has(refId)) errors.push(`test_strategy_matrix references missing reference id: ${refId}`);
}
const strategyLevels = new Set();
for (const level of testStrategyMatrix?.levels ?? []) {
  assertExactFields("test strategy level", level.level ?? "unknown", level, TEST_STRATEGY_LEVEL_FIELDS);
  if (strategyLevels.has(level.level)) errors.push(`Duplicate test strategy level: ${level.level}`);
  strategyLevels.add(level.level);
  for (const field of ["level", "name", "goal_zh", "goal_en"]) {
    assertNonEmptyString("test strategy level", level.level ?? "unknown", field, level[field]);
  }
  assertUniqueArray("test strategy level", level.level ?? "unknown", "typical_checks", level.typical_checks);
}

assertExactFields("tdd workflow", tddWorkflow?.workflow_id ?? "unknown", tddWorkflow ?? {}, TDD_WORKFLOW_FIELDS);
if (tddWorkflow?.schema_version !== "1.0.0") errors.push("tdd_workflow schema_version must be 1.0.0");
if (tddWorkflow?.workflow_id !== "workflow.skill_tdd") errors.push("tdd_workflow workflow_id mismatch");
const tddStepIds = new Set();
for (const step of tddWorkflow?.steps ?? []) {
  assertExactFields("tdd workflow step", step.id ?? "unknown", step, TDD_WORKFLOW_STEP_FIELDS);
  if (tddStepIds.has(step.id)) errors.push(`Duplicate tdd workflow step id: ${step.id}`);
  tddStepIds.add(step.id);
  if (!TDD_PHASES.has(step.phase)) errors.push(`Invalid tdd workflow phase for ${step.id}: ${step.phase}`);
  for (const field of ["id", "phase", "action_zh", "action_en"]) {
    assertNonEmptyString("tdd workflow step", step.id ?? "unknown", field, step[field]);
  }
  assertUniqueArray("tdd workflow step", step.id ?? "unknown", "outputs", step.outputs);
}

assertExactFields("tdd planning matrix", tddPlanningMatrix?.matrix_id ?? "unknown", tddPlanningMatrix ?? {}, TDD_PLANNING_FIELDS);
if (tddPlanningMatrix?.schema_version !== "1.0.0") errors.push("tdd_planning_matrix schema_version must be 1.0.0");
if (tddPlanningMatrix?.matrix_id !== "testing.skill_tdd_planning") errors.push("tdd_planning_matrix matrix_id mismatch");
assertUniqueArray("tdd planning matrix", tddPlanningMatrix?.matrix_id ?? "unknown", "reference_ids", tddPlanningMatrix?.reference_ids ?? []);
for (const refId of tddPlanningMatrix?.reference_ids ?? []) {
  if (!knownRefIds.has(refId)) errors.push(`tdd_planning_matrix references missing reference id: ${refId}`);
}
const planningPhases = new Set();
for (const phase of tddPlanningMatrix?.phases ?? []) {
  assertExactFields("tdd planning phase", phase.phase ?? "unknown", phase, TDD_PLANNING_PHASE_FIELDS);
  if (planningPhases.has(phase.phase)) errors.push(`Duplicate tdd planning phase: ${phase.phase}`);
  planningPhases.add(phase.phase);
  if (!TDD_PHASES.has(phase.phase)) errors.push(`Invalid tdd planning phase: ${phase.phase}`);
  assertNonEmptyString("tdd planning phase", phase.phase ?? "unknown", "stop_condition", phase.stop_condition);
  assertUniqueArray("tdd planning phase", phase.phase ?? "unknown", "required_outputs", phase.required_outputs);
}
