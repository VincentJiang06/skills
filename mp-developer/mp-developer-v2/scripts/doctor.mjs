...
export function resolveProject(projectDir) {
  ...
  // Valid JSON that isn't a config OBJECT (null, array, primitive) is malformed — reject cleanly,
  // never dereference it (F2: `null` config parsed then threw on cfg.miniprogramRoot). {} stays valid.
  if (cfg === null || typeof cfg !== 'object' || Array.isArray(cfg)) return { ok: false, reason: 'BAD_PROJECT_CONFIG', mpRoot: null }
  // A present-but-non-string miniprogramRoot is a malformed VALUE — reject before .replace() throws
  // (F3: number/array/object miniprogramRoot). Absent (undefined) is fine → defaults to project root.
  if (cfg.miniprogramRoot !== undefined && typeof cfg.miniprogramRoot !== 'string') return { ok: false, reason: 'BAD_PROJECT_CONFIG', mpRoot: null }
...
export function checkFreshness(mpRoot) {
...
export function checkTypecheck(projectDir, mpRoot, skip) {
...
export function runDoctor({ project, skipTypecheck }) {
  const checks = {}
