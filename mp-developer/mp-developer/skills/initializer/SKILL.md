...
其中 `<skillVersionFromSkillYaml>` 不是固定字符串；调用前应读取当前调用侧 agent 加载的 `skill.yaml`，取该文件顶层 `version` 字段的值填入。

| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `version` | string | 否 | 当前调用侧 agent 加载的 `skill.yaml` 文件中的 `version` 字段值；不传则跳过版本对比提醒 |
