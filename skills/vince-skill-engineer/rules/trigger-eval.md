...
It is shared tooling: `vince-skill-guidance` cites it as the way to satisfy the
metrics pillar, and `vince-skill-zipper`'s Retrigger operation uses it to prove a
...
node ../vince-skill-engineer/scripts/trigger_eval.mjs <skill-dir> cases.json --judge cli --threshold 0.9 --json

# Deterministic self-test of the runner's math (no tokens, used by evals/run_all.mjs):
node ../vince-skill-engineer/scripts/trigger_eval.mjs <skill-dir> cases.json --judge mock --mock-rule YES
```
