...
if (fs.existsSync(path.join(root, "agents/openai.yaml"))) {
  ...
  if (!openai.includes("$mp-cli-sup")) {
    errors.push("agents/openai.yaml default_prompt must mention $mp-cli-sup");
  }
}
...
for (const file of ["assets/skill-design-record.json", "assets/metric-plan.json", "assets/release-manifest.json"]) {
  ...
  if (parsed && !String(parsed.skill_id || parsed.skill?.id || "").includes("mp-cli-sup")) {
    errors.push(`${file} must identify mp-cli-sup`);
  }
}
