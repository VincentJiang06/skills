...
| **WeChat mini-program** (`.wxml`/`.wxss`) | WeChat DevTools automation (sibling skill **vince-mp-cli-sup**) for screenshots + runtime probes — a plain browser cannot render it. | Pass the page's `.wxss` to `analyze.py --css` for the static numeric checks; use the screenshot for the visual long tail. |
