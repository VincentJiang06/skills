...
**好在哪** ——
- 一张扎根 `skill-principle` 知识库的 **7 支柱就绪度评分表**，判断 skill 是否「工业级」、缺什么、缺多少。
...
- **v2.0：spec 门是可执行的** —— 产出前必须过 `validate_spec.mjs`（恰 7 支柱、评分↔状态、verdict 与 cap 规则、gap→action 全查，exit 0 才算完）；新增 audit disposition：终验复审写 `post-build-audit.json`，不再覆写构建 spec。
...
**安装** —— `npx skills add VincentJiang06/skills`（或 `cp -R skills/skill-guidance ~/.claude/skills/`）。`skill-principle/` 知识库已内置在本 skill 目录里，会随 `skill-guidance` 一起安装。
