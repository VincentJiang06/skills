...
`skill-principle` is an agent-first knowledge base for industrial skill development.
...
Embedded KB for `skill-guidance`: `skills/skill-guidance/skill-principle/`.
`skill-engineer` and `skill-conductor` reuse it via
`../skill-guidance/skill-principle`.
...
`query_kb.mjs` returns matching docs, nodes, assets, references, summaries, and
expansion hints.
...
`run_all_checks.mjs` validates structure, index freshness, query routing,
context budgets, and doc traceability.
