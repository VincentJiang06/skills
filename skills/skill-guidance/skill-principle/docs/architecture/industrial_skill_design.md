...
reference_ids: `ref.anthropic.equipping_agents_skills`, `ref.microsoft.agent_skills`, `ref.anthropic.building_effective_agents`, `ref.anthropic.writing_tools_for_agents`, `ref.anthropic.skill_authoring_best_practices`, `ref.openai.codex_best_practices`, `ref.toolformer`, `ref.fable.skill_engineering_synthesis_2026_07`, `ref.claude_code.skill_listing_lifecycle`, `ref.claude_code.invocation_controls`, `ref.agentskills.open_standard_2025_12`

node_ids: `pillar.design`, `concept.skill`, `concept.trigger`, `concept.execution_protocol`, `principle.progressive_disclosure`, `principle.executable_acceptance`, `principle.claim_evidence_traceability`, `anti_pattern.prompted_architecture`, `principle.description_is_the_index`, `anti_pattern.sibling_trigger_overlap`, `principle.rule_at_point_of_use`
...
## 9. 触发与指令的机制细节 / Trigger and instruction mechanics

上面第 3、4、5 节的原则在实现层还有三条可操作机制（来源见 `ref.fable.skill_engineering_synthesis_2026_07`；触发承重数字见 Fact Registry）：

- `principle.description_is_the_index`：frontmatter description 是技能唯一常驻上下文的文本，它就是检索索引项——触发是一个 IR 问题，不是散文问题。按 what + when/触发词 + 一条关键 do-NOT 来写，正文再好也救不了一条永不命中的 description。Claude Code 的 listing 预算与降级机制（`ref.claude_code.skill_listing_lifecycle`）意味着：库越大，最少调用技能的 description 会被优先丢弃，触发力静默衰减。
- `anti_pattern.sibling_trigger_overlap`：同一族技能相互抢触发，过触发常表现为族内错的成员接走请求。对策是每条 description 写明负向路由（NOT for X → sibling-Y），并在含近邻请求的 holdout 集上测精确率。需要彻底禁止模型触发的技能，可用 `ref.claude_code.invocation_controls` 的 `disable-model-invocation`。
- `principle.rule_at_point_of_use`：指令的约束力随离动作的距离衰减，序言里的规则到几千 token 后的执行点常输给局部上下文。把每条规则放在它管辖的步骤旁边（rule-then-rationale），序言只留全局不变量。

These are the implementation-layer mechanics behind sections 3–5. The description IS the retrieval index (triggering is an IR problem); sibling skills in a family steal each other's activations, so give each description explicit negative routing and measure precision on a near-neighbor holdout; and an instruction's grip decays with distance, so place each rule next to the step it governs. Exact char/token limits stay in the Fact Registry.
