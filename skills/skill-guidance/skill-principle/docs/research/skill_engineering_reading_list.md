...
- `ref.anthropic.equipping_agents_skills` (tier 1) — **Equipping agents for the real world with Agent Skills**
  - Anthropic's canonical definition of an Agent Skill: an organized folder of instructions, scripts, and resources discovered and loaded on demand, with progressive disclosure (only name+description preloaded) as the load-bearing design principle. The definitional anchor for treating a skill as a composable capability package, not a prompt.
  - 用途: Agent Skills 的权威首发定义与设计哲学——skill 是按需发现/加载的指令+脚本+资源文件夹，渐进披露是承重原则。作为「skill 是可组合能力包而非提示词」的定义锚点。
- `ref.anthropic.skill_authoring_best_practices` (tier 1) — **Skill authoring best practices**
  - First-party authoring guide with the decisions that move the needle: the description is the trigger Claude uses to choose among 100+ skills (state WHEN to use it), keep the body concise (context is a public good), respect the name/description length limits, split SKILL.md into referenced files as it grows, and test on real tasks. The canonical reference for trigger wording and file structure.
  - 用途: 官方编写指南，最影响成败的决策——description 即触发器，必须写清「何时使用」；正文精简；遵守 name/description 长度限制；SKILL.md 变大就拆分；用真实任务测试。触发措辞与文件结构的权威参考。
