...
reference_ids: `ref.prisma.2020`, `ref.cochrane.handbook`, `ref.grade.working_group`, `ref.nato.admiralty_code`, `ref.caulfield.sift`, `ref.unaids.triangulation`, `ref.w3c.prov_overview`, `ref.arxiv.self_rag`, `ref.arxiv.deeprag`, `ref.arxiv.agentic_rag_survey`, `ref.arxiv.question_decomp_rag`, `ref.arxiv.deepwidesearch`, `ref.arxiv.claimaudit`, `ref.pirolli.information_foraging`, `ref.fable.skill_engineering_synthesis_2026_07`

node_ids: `pillar.research`, `procedure.source_roster`, `procedure.search_fan_out`, `procedure.iterative_deepening`, `procedure.source_reliability_grading`, `procedure.triangulation`, `principle.claim_evidence_traceability`, `metric.source_coverage`, `metric.claim_traceability_rate`, `procedure.model_knowledge_sourcing`
...
## 10. 模型内生知识作为信源 / Sourcing model-internal knowledge

`procedure.model_knowledge_sourcing`（来源 `ref.fable.skill_engineering_synthesis_2026_07`）：前沿模型自身也可作为 KB 信源，但要走一套纪律，避免把不可溯源的自信当成事实——

- `source_type` 标 `model_synthesis`，写明模型名与撰写日期。
- 需 owner 显式授权（本 KB 的授权记录见 `decisions/0002-model-synthesis-source.md`）。
- `reliability_notes` 如实声明"未经外部独立网络验证"。
- 机制性（怎么做、为什么会坏）知识可定 tier-2；任何具体数字一律用时再核，承重数字以 UPDATE.md 的 Fact Registry 为单一事实源。
- 当某条主张变成 load-bearing（要据它收紧某道门）时，先升级：找 tier-1 一手源交叉印证再动门。

这条纪律是可靠性分级（`procedure.source_reliability_grading`）在"信源是模型本身"这一特例上的落地，输出仍受声明可溯源（`principle.claim_evidence_traceability`）约束。

A frontier model's own knowledge may serve as a KB source under a discipline: mark source_type model_synthesis, name the model and date, require explicit owner authorization, state plainly that it is not independently web-verified, and defer every specific number to the Fact Registry. When a claim becomes load-bearing, upgrade it to a tier-1 primary source before tightening any gate on it. This is reliability grading applied to the special case where the source is the model itself.
