...
machine_summary_zh: skill-principle 维护一个最著名、用户量最多的公共 skill 仓库注册表，需要时按需下载某个具体 skill 作为学习参考——学它的内容组织和重点，但工业级 skill 在结构上要考虑得更多。

machine_summary_en: skill-principle keeps a registry of the most popular public skill repositories and fetches a specific skill on demand as a learning reference — learn its content and emphasis, but an industrial skill must consider more structure.
...
工具通过 GitHub raw / `gh api` 拉取指定路径，不写入 `skill-principle/` 内部（否则会破坏 KB 的文件清单校验）。缓存是临时的、可删的学习材料，不是 KB 的一部分。
