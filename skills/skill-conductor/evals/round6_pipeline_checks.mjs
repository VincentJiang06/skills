...
const buildReportSchema = json("skill-engineer/assets/build-report.schema.json");
const conductorLogSchema = json("skill-conductor/assets/conductor-log.schema.json");
const handoffSchema = json("skill-guidance/assets/handoff-spec.schema.json");
const handoffExample = json("skill-guidance/assets/handoff-spec.example.json");
const pipelineLoop = read("skill-conductor/rules/pipeline-loop.md");
const finalAcceptance = read("skill-conductor/rules/final-acceptance.md");
const conductorSkill = read("skill-conductor/SKILL.md");
...
check(
  ...
  /Pipeline mode|skill-conductor|Stage Z/i.test(zipperSkill),
...
const budgets = {
  "skill-guidance": 1700,
  "skill-engineer": 1820,   // +trigger_eval (script+rule+schema) registration row
  "skill-conductor": 1900,
  [zipperRel]: 1500
};
...
const documentedDeps = [
  ...
  "skill-zipper/scripts/diff_lossless.py",
  "skill-zipper/scripts/measure_tokens.py",
...
const staleZipperPath = ["chore-develop", "skill-zipper"].join("-");
for (const [rel, text] of Object.entries({
  "skill-conductor/SKILL.md": conductorSkill,
  "skill-conductor/rules/pipeline-loop.md": pipelineLoop,
  "skill-guidance/assets/handoff-spec.example.json": JSON.stringify(handoffExample)
})) {
  check(
