...
Run three derivation engines together over `assets/payload-library.json`:
...
cost/likelihood/prereq; attack cheapest-highest-impact first; **stop on budget**
(`--budget N`, rolled up as `ASR@n`).
...
`ASR@n` + unique-finding count + severity histogram. Validate with
