...
description: >
  Design the engineered loop for one concrete (semi-)autonomous AI-coding task
  and emit a filled, machine-checkable loop-design spec. It DESIGNS and teaches
  loop design; it does NOT execute the loop. Applied front-end over the
  loop-principle KB: retrieves from it and reuses its templates/checklists
  instead of restating principles. Anchor: a fast machine-runnable check closes
  a loop autonomously, so design backward from "what check proves this is done?"
...
Design the engineered loop for one concrete (semi-)autonomous coding task and
emit a **filled, machine-checkable loop-design JSON** that
`scripts/lint_loop_design.mjs` PASSes.
...
2. Restate the concrete target task in **one line**.
3. Resolve the KB path and warm up retrieval:
...
`references/loop-principle-map.md`). Fill the canonical loop-design object as you
go (shape in `references/loop-design-shape.md`; the linter binds to those keys).
...
3. **Feedback signal** — the cheapest runnable check on the spectrum
   ...
   → `concept.feedback_signal_spectrum`, `principle.closed_loop_needs_a_check`
...
9. **Emit + self-score** — output the filled loop-design JSON (optionally
   rendered via `loop-principle/templates/loop_design.template.md`), plus a
   self-scored rubric (`loop-principle/templates/loop_quality_rubric.template.json`)
   and residual risks.
...
| File | When to load |
|------|--------------|
| `references/loop-principle-map.md` | The 9 steps → loop-principle node ids + docs + which templates/checklists to reuse, and the query_kb recipe. |
| `references/loop-design-shape.md` | The exact canonical loop-design JSON keys the linter validates (build the object to this shape). |
| `scripts/lint_loop_design.mjs` | The deterministic verifier of a produced loop-design. CLI (`node ... <design.json>` / stdin) or `import { validate }`. |
| `assets/golden-loop-design.json` | A fully filled loop-design the linter PASSes — copy it as the starting fixture. |
| `evals/run_all.mjs` | Re-runnable adversarial battery (12 cases) over the linter; `node evals/run_all.mjs`. |

## Controls

- **Design-only.** Never execute the designed loop, run the target's code, or
  modify the loop-principle KB.
- **Reject-on-no-check.** A loop-design with no runnable feedback signal is
  rejected (the linter FAILs with a field-naming reason).
- **Mandatory cap.** Every emitted design includes `stop_conditions.max_iterations`
  and a non-empty `stop_conditions.failure` list.
