...
Downstream PR outcomes and long-horizon benchmarks should be tracked separately from local task success. A 2026 PR study found that fix-related agent PRs still fail to merge for mundane integration reasons such as failing tests and incomplete fixes, so `metric.regression_escape_rate` should include CI/review outcomes. SWE-EVO adds the complementary warning: strong SWE-bench Verified performance does not automatically transfer to long-horizon software evolution, so benchmark scores must be labeled by task horizon and repository-evolution scope.
