...
Persist the report as `loop-principle/`, mirroring the `skill-principle` architecture (a 网状 / mesh structure):
...
- **Generated indexes** (`indexes/*`) are built by `tools/build_indexes.mjs` (copied unchanged from skill-principle — it is path-relative): search index, graph adjacency, route map, summary cards.
