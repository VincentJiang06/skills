...
of the ratio — record it as a P0 gap.
...
| `low_context_kb` | `低上下文 渐进披露 短入口 模块 加载` | Short SKILL.md (<500 lines), progressive disclosure into `rules/`/`references/`, on-demand loading with clear pointers. | Everything stuffed in SKILL.md; no module split; always-loaded bloat. |
...
2. Compare the target's SKILL.md + dirs (use the `scripts/score_skill.mjs`
   `pillar_hints` as a starting seed, then verify by reading).
...
could be better." Every gap must be actionable by `vince-skill-engineer`.
