...
3. For an empirical check (not just eyeballing), run the engineer's trigger eval
   on a labeled set:
   `node ../vince-skill-engineer/scripts/trigger_eval.mjs <skill-dir> cases.json --judge cli --threshold 0.9`
   It scores trigger precision/recall; use the spec's `Do NOT use` neighbours as
   the `should_trigger: false` cases. See `../vince-skill-engineer/rules/trigger-eval.md`.
