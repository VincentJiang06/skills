# vince-hifi-review Implementation Plan
...
**Spec:** `docs/superpowers/specs/2026-06-02-vince-hifi-review-design.md`
**Branch:** `2-vince-hifi-review` (already created off `main`; merge back in the final task).
**Build dir:** `2-vince-hifi-review/` (repo root). All paths below are relative to it unless noted.
...
- **Commit style**: `type(2-vince-hifi-review): subject`, ending with the Co-Authored-By trailer. Commit after every green step.
...
**Files:**
- Create: `2-vince-hifi-review/` and all subdirs.

- [ ] **Step 1: Make directories**

Run (from repo root `/Users/vince/playground/skill-developer`):
```bash
mkdir -p 2-vince-hifi-review/{rules,references,scripts,schemas,evals/fixtures,meta,agents}
```

- [ ] **Step 2: Verify**

Run: `find 2-vince-hifi-review -type d | sort`
Expected: the 8 directories above, no errors.

- [ ] **Step 3: Commit**

```bash
git add 2-vince-hifi-review
git commit -m "chore(2-vince-hifi-review): scaffold package directories"
```
...
Run: `python3 -c "import json;json.load(open('2-vince-hifi-review/references/source-registry.json',encoding='utf-8'));print('ok')"`
Expected: `ok`
...
- [ ] **Step 3: Validate parses** → `python3 -c "import json;json.load(open('2-vince-hifi-review/references/band-taxonomy.json',encoding='utf-8'));print('ok')"` → `ok`.

- [ ] **Step 4: Commit**
```bash
git add 2-vince-hifi-review/references/band-taxonomy.json 2-vince-hifi-review/schemas/band-taxonomy.schema.json
git commit -m "feat(2-vince-hifi-review): band taxonomy + quanta scale (single source of truth)"
```
...
- [ ] **Step 4: Commit**
```bash
git add 2-vince-hifi-review/references/targets.json 2-vince-hifi-review/schemas/targets.schema.json
git commit -m "feat(2-vince-hifi-review): target band-balance table + schema"
```
...
- [ ] **Step 4: Commit**
```bash
git add 2-vince-hifi-review/references/source-gear-thresholds.json 2-vince-hifi-review/schemas/source-gear-thresholds.schema.json
git commit -m "feat(2-vince-hifi-review): source-gear competence + matching thresholds"
```
...
- [ ] **Step 3: Commit**
```bash
git add 2-vince-hifi-review/references/signature-glossary.md 2-vince-hifi-review/references/research-bibliography.md
git commit -m "docs(2-vince-hifi-review): bilingual glossary + research bibliography"
```
...
Run: `python3 -c "import sys;sys.path.insert(0,'2-vince-hifi-review/evals');from schema_check import validate_file as v;print(v('2-vince-hifi-review/references/band-taxonomy.json','2-vince-hifi-review/schemas/band-taxonomy.schema.json'))"`
Expected: `[]` (empty = valid)

- [ ] **Step 3: Commit**
```bash
git add 2-vince-hifi-review/evals/schema_check.py
git commit -m "test(2-vince-hifi-review): stdlib json-schema subset validator"
```
...
- [ ] **Step 5: Commit**
```bash
git add 2-vince-hifi-review/evals/fixtures/
git commit -m "test(2-vince-hifi-review): transducer FR fixtures + golden skeletons"
```
...
Run: `python3 -c "import sys;sys.path.insert(0,'2-vince-hifi-review/evals');from schema_check import validate_file as v;print(v('2-vince-hifi-review/evals/eval-cases.json','2-vince-hifi-review/schemas/eval-cases.schema.json'))"`
Expected: `[]`

- [ ] **Step 6: Commit**
```bash
git add 2-vince-hifi-review/evals/fixtures/ 2-vince-hifi-review/evals/eval-cases.json 2-vince-hifi-review/schemas/eval-cases.schema.json
git commit -m "test(2-vince-hifi-review): source-gear fixtures + eval-cases + schema"
```
...
git add 2-vince-hifi-review/scripts/fr_analyze.py 2-vince-hifi-review/schemas/fr-analysis.schema.json 2-vince-hifi-review/evals/fixtures/warm_iem.expected.json 2-vince-hifi-review/evals/fixtures/vshape_tws.expected.json 2-vince-hifi-review/evals/fixtures/neutral_hp.expected.json
git commit -m "feat(2-vince-hifi-review): transducer FR engine + frozen goldens"
```
...
```bash
cd /Users/vince/playground/skill-developer
git add 2-vince-hifi-review/scripts/source_analyze.py 2-vince-hifi-review/schemas/source-analysis.schema.json 2-vince-hifi-review/evals/fixtures/transparent_dac.expected.json 2-vince-hifi-review/evals/fixtures/weak_amp_300ohm.expected.json
git commit -m "feat(2-vince-hifi-review): source-gear engine + frozen goldens"
```
...
Run: `cd 2-vince-hifi-review && python3 scripts/validate_output.py evals/fixtures/eval_good.json; echo "exit=$?"`
...
```bash
cd /Users/vince/playground/skill-developer
git add 2-vince-hifi-review/schemas/evaluation.schema.json 2-vince-hifi-review/scripts/validate_output.py 2-vince-hifi-review/evals/fixtures/eval_good.json 2-vince-hifi-review/evals/fixtures/eval_bad_untraced.json
git commit -m "feat(2-vince-hifi-review): evaluation output contract + traceability gate"
```
...
Run: `cd 2-vince-hifi-review && python3 evals/run_all.py; echo "exit=$?"`
...
```bash
cd /Users/vince/playground/skill-developer
git add 2-vince-hifi-review/evals/run_all.py
git commit -m "test(2-vince-hifi-review): L0/L1/determinism/gate regression runner"
```

### Task 14: `SKILL.md` (always-loaded entry, < 900 tokens)

**Files:**
- Create: `2-vince-hifi-review/SKILL.md`
...
```bash
cd /Users/vince/playground/skill-developer
git add 2-vince-hifi-review/SKILL.md
git commit -m "feat(2-vince-hifi-review): SKILL.md entry (protocol + module/script tables)"
```
...
- [ ] **Step 9: Commit**
```bash
git add 2-vince-hifi-review/rules/
git commit -m "docs(2-vince-hifi-review): eight on-demand rules files"
```
...
```json
{
  ...
  "skill_id": "skill.2-vince-hifi-review",
  "required_metrics": [
...
- [ ] **Step 4: Commit**
```bash
git add 2-vince-hifi-review/meta/
git commit -m "docs(2-vince-hifi-review): meta four-piece (design record, metric plan, release gates)"
```
...
- [ ] **Step 1: Write `agents/openai.yaml`** — mirror the sibling skill's file (open `/Users/vince/.claude/skills/vince-low-visibility-fix/agents/openai.yaml`, copy its shape, swap name/description to `2-vince-hifi-review` and a one-line summary).
...
- [ ] **Step 4: Commit**
```bash
git add 2-vince-hifi-review/agents 2-vince-hifi-review/CHANGELOG.md 2-vince-hifi-review/README.md
git commit -m "chore(2-vince-hifi-review): packaging (openai branding, changelog, readme)"
```
...
```bash
git add 2-vince-hifi-review/references 2-vince-hifi-review/evals/fixtures
git commit -m "fix(2-vince-hifi-review): validate target/threshold values vs research"
```
...
- [ ] **Step 1: Routing test** — for each `evals/eval-cases.json` case, dispatch a fresh subagent with only the case `note` as the user turn and ask whether `2-vince-hifi-review` should activate. Record predicted vs `must_activate`. Compute activation precision/recall.
...
- [ ] **Step 4: Commit**
```bash
git add 2-vince-hifi-review/meta/live-eval-results.json
git commit -m "test(2-vince-hifi-review): live activation + behavioral evals"
```
...
- [ ] **Step 1: Run the full regression once more** → `cd 2-vince-hifi-review && python3 evals/run_all.py` → GREEN.
...
```bash
git add 2-vince-hifi-review/meta/release-checklist.json
git commit -m "chore(2-vince-hifi-review): mark release gates pass for 0.1.0"
```
...
```bash
git checkout main
git merge --no-ff 2-vince-hifi-review -m "merge(2-vince-hifi-review): objective hifi review skill 0.1.0"
```

- [ ] **Step 5: Verify the skill loads** — confirm `2-vince-hifi-review/SKILL.md` frontmatter `name` is accepted by Claude Code. If a leading digit is rejected, set frontmatter `name: vince-hifi-review` (keep the folder name), commit, and note in CHANGELOG.
